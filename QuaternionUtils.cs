...
public static class QuaternionUtil
{
    ...
    public static Vector3 RotVecByQuat(Quaternion q, Vector3 v)
    {
        Quaternion p = new Quaternion(v.x, v.y, v.z, 0.0f);
        Quaternion pPrime = (q * p) * Quaternion.Inverse(q);
    ...
    public static Quaternion rotateQuaternion(Quaternion q, Quaternion qRot)
    {
        q.ToAngleAxis(out float angle, out Vector3 axis);
        Quaternion axisQ = new Quaternion(axis.x, axis.y, axis.z, 0.0f);
        Quaternion rotatedAxis = qRot * axisQ * Quaternion.Inverse(qRot);
        
        Vector3 newAxis = new Vector3(rotatedAxis.x, rotatedAxis.y, rotatedAxis.z);
        newAxis = (float)Math.Sin(angle / 2.0f) * newAxis;
        float newAngle = (float)Math.Cos(angle / 2.0f);
        
        return new Quaternion(newAxis.x, newAxis.y, newAxis.z, (float)newAngle);
    }
}